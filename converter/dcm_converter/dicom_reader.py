...
class Dicom_Reader(object):
    ...
    def draw_contours(self, structure, annotation_list):
        ...
        for i in range(len(structure.ROIContourSequence)):
            ...
            flag, index = self.has_annotation(ROIName,annotation_list)
            if flag:
                contour_item = {}
                contour_item['number'] = structure.ROIContourSequence[i].ReferencedROINumber
                contour_item['name'] = annotation_list[index]
    ...
    def has_annotation_fuzzy(self, annotation, annotation_list):
        import difflib
        sim_list = []
    ...
    def has_annotation(self, annotation, annotation_list):
        ...
        if len(annotation_list) == 1:
            ...
            return self.has_annotation_fuzzy(annotation,annotation_list)
    ...
    def draw_labels(self, shape, contours, meta_data, annotation_list):
        ...
        for annotation in annotation_list:
            ...
            if 'coord_point' not in con or len(con['coord_point']) == 0:
                print('There is no point in %s!'%annotation)
                continue
