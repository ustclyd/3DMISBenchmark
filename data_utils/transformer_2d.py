...
class RandomErase2D(object):
    ...
    def __call__(self, sample):
        ...
        # map_array = np.ones(image.shape,dtype=np.uint8)
        ...
        # map_array[roi_window[0][0]:roi_window[0][1],roi_window[1][0]:roi_window[1][1]] = 0
        # index_array = np.transpose(np.nonzero(map_array))
        # erase_center = index_array[random.choice(range(index_array.shape[0]))]

        # erase_h_s = max(0,(erase_center[0] - max_h//2))
        # erase_h_e = min(h,(erase_center[0] + max_h//2))
        # erase_w_s = max(0,(erase_center[1] - max_w//2))
        # erase_w_e = min(w,(erase_center[1] + max_w//2))
        # image[erase_h_s:erase_h_e,erase_w_s:erase_w_e] = 0
